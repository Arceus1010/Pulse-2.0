  ...
  if (sources.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-2 px-8 py-16 text-center">
        <p className="text-sm font-medium text-slate-500 dark:text-zinc-400">No sources</p>
        <p className="text-xs text-slate-500 dark:text-zinc-400">No sources were collected for this task.</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col">
      ...
      {sorted.length === 0 ? (
        <div className="flex items-center justify-center px-8 py-12">
          <p className="text-xs text-slate-500 dark:text-zinc-400">No sources match the selected filters.</p>
        </div>
      ) : (
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-zinc-800/50">
              <th className="w-9 px-3 py-2.5 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-zinc-400 border-b border-slate-100 dark:border-zinc-800">
                #
              </th>
              <th className="px-3 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-zinc-400 border-b border-slate-100 dark:border-zinc-800">
                Title
              </th>
              <SortableHeader
                label="Kind"
...
function SortableHeader({ label, field, current, dir, onSort }: {
  ...
  return (
    <th
      onClick={onSort}
      className={`px-3 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider border-b border-slate-100 dark:border-zinc-800 cursor-pointer select-none transition-colors whitespace-nowrap ${
        isActive
...
function SourceRow({ source, rowNumber, expanded, onToggle }: {
  ...
  return (
    <>
      ...
      >
        {/* # */}
        <td className="px-3 py-2.5 text-right text-[11px] tabular-nums text-slate-500 dark:text-zinc-400 align-top">
          {rowNumber}
        </td>
        ...
        <td className="px-3 py-2.5 align-top max-w-0">
          <div className="flex items-start justify-between gap-1.5">
            <p className="text-xs font-medium text-slate-700 dark:text-zinc-200 leading-relaxed truncate">
              {source.title}
            </p>
        ...
        <td className="px-3 py-2.5 align-top max-w-0">
          {source.discardReason ? (
            <p
              className="text-xs text-slate-500 dark:text-zinc-400 leading-relaxed line-clamp-2"
    ...
    <div className="px-12 py-4 bg-blue-50/20 dark:bg-blue-950/5 border-t border-blue-100/60 dark:border-blue-900/20 flex flex-col gap-3.5">
      ...
      <div className="flex flex-col gap-1">
        ...
        {source.uri ? (
          <a
            ...
            className="inline-flex items-center gap-1 text-xs text-blue-700 dark:text-blue-400 hover:underline break-all"
          >
            {source.uri}
      ...
      {source.excerpt && (
        <div className="flex flex-col gap-1">
          ...
          <blockquote className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed border-l-2 border-slate-200 dark:border-zinc-700 pl-3 italic">
            "{source.excerpt}"
          </blockquote>
        </div>
      )}
...
function KindBadge({ kind }: { kind: SourceKind }) {
  ...
  return (
    <span className={`inline-flex items-center text-[11px] font-semibold px-1.5 py-0.5 rounded border ${styles[kind]}`}>
      {KIND_LABELS[kind]}
    </span>
  )
}
...
function RelevanceCell({ score }: { score: number | null }) {
  if (score === null) {
    return <span className="text-[11px] text-slate-400 dark:text-zinc-500">—</span>
  }
  return (
    <div className="flex flex-col gap-1.5 w-12">
      <span className="text-xs tabular-nums text-slate-700 dark:text-zinc-300 leading-none">{score}</span>
