    ...
    <div className="flex-1 flex flex-col px-5 py-6 gap-6">
          ...
          {editing ? (
            ...
            <ol className="flex flex-col gap-2">
              {subQuestions.map((q, i) => (
                <li key={i} className="flex gap-2.5 text-sm text-slate-700 dark:text-zinc-300 leading-relaxed">
            ...
            {task.sourcesWeb && (
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-medium text-slate-500 dark:text-zinc-400">Web search</span>
                </div>
                  ...
                  <ul className="flex flex-col gap-1">
                    {webQueries.map((q, i) => (
                      <li key={i} className="flex items-start gap-2">
                        ...
                        <span className="text-sm text-slate-600 dark:text-zinc-400 font-mono leading-relaxed break-all">
                          {q}
                        </span>
                      </li>
                    ))}
            ...
            {task.sourcesKb && (
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-medium text-slate-500 dark:text-zinc-400">Knowledge base</span>
                </div>
      ...
      {outputs?.strategy && (
        <section className="flex flex-col gap-2">
          ...
          <p className="text-sm text-slate-600 dark:text-zinc-300 leading-relaxed bg-slate-50 dark:bg-zinc-800/60 rounded-md px-3 py-2.5">
            {outputs.strategy}
          </p>
...
function SectionHeader({ label, badge }: { label: string; badge?: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      {badge && (
        <span className="text-xs font-medium tabular-nums px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400">
          {badge}
        </span>
      )}
    </div>
    ...
    <div className="flex flex-col gap-1.5">
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-1.5">
          <input
            ...
            className={`flex-1 min-w-0 h-7 px-2.5 rounded-sm border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-slate-800 dark:text-zinc-100 placeholder:text-slate-500 dark:placeholder:text-zinc-400 outline-none focus:border-blue-300 dark:focus:border-blue-700/60 transition-colors text-base ${
              mono ? 'font-mono' : ''
            }`}
          />
          <button
      ...
      {items.length < maxItems && (
        <button
          type="button"
          onClick={add}
          className="flex items-center gap-1 self-start text-sm text-slate-500 dark:text-zinc-400 hover:text-slate-600 dark:hover:text-zinc-300 transition-colors mt-0.5"
        >
